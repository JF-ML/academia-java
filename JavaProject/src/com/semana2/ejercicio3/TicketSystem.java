package com.semana2.ejercicio3;
...
class Ticket {
    ...
    public void transitionTo(TicketStatus newStatus) {
    	...
    	if (!status.canTransitionTo(newStatus)) 
    		System.out.println("No se puede transicionar de "+this.status+" a "+newStatus);
    	else {
    		System.out.println("Ticket "+this.id+": "+this.status+" -> "+newStatus);
...
public class TicketSystem {
    public static void main(String[] args) {
        ...
        for (Ticket ticket : tickets) {
        	if(conteo.containsKey(ticket.getStatus())) {
        		conteo.replace(ticket.getStatus(), conteo.get(ticket.getStatus())+1);
        ...
        conteo.forEach((status, count) ->
            System.out.printf("X  %s: %d%n", status, count));
